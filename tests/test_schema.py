...
def test_all_planned_tables_exist(conn):
...
def _insert_company(conn, cid="c1"):
    conn.execute(
...
def _insert_deal(conn, did="d1", cid="c1"):
    conn.execute(
...
def test_enums_are_enforced(conn):
...
def test_confidence_bounds(conn):
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            """INSERT INTO memories(id, type, subject, content, confidence, valid_from, created_by, created_at, updated_at)
...
def test_evidence_memory_provenance_roundtrip(conn):
    _insert_company(conn)
    _insert_deal(conn)
    conn.execute(
...
def test_evidence_full_text_search(conn):
    conn.execute(
...
def test_action_dedupe_by_source(conn):
    conn.execute(
